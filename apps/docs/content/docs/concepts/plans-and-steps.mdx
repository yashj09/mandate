...
Two recipes ship: `recipes.liquidity` (borrow, bridge, pay, schedule) and `recipes.repayment` (bridge back if needed, repay, withdraw collateral, mark repaid). You can also call `client.plan(requests, opts)` with your own list of action requests.
